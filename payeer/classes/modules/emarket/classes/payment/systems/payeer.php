...
class payeerPayment extends payment 
{
	...
	{	
		...
		$param['payeer_desc']	= base64_encode('Payment order No. ' . $param['payeer_orderid']);
		...
		if ($valid_ip)
		{
			...
			$FORMS = "<form action=" . $param['payeer_url'] . " method='get' name='form_payeer'>
				...
				<input type='submit' name='m_process' value='Оплатить' class='button big' />
			</form>
		...
		else
		{
			...
			$FORMS_F = Array();
			$FORMS_F = "<form action='/emarket/purchase/result/failed/' method='get' name='form_payeer_fail'>
				<input type='hidden' name='m_orderid' value=" . $param['payeer_orderid'] . " />
			</form>
			<script type='text/javascript'>document.form_payeer_fail.submit();</script>";
    ...
    public function poll() 
	{
		...
		if (isset($_POST["m_operation_id"]) && isset($_POST["m_sign"]))
		{
			...
			if ($_POST["m_sign"] == $sign_hash && $_POST['m_status'] == "success")
			{
				...
				$this->order->payment_document_num = $vars->trans_id;
